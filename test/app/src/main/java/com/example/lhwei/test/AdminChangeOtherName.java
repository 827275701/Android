 ...
 */
public class AdminChangeOtherName extends Activity{
    EditText Eno;
    EditText Enew_name;
    Button submit;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.admin_change_other_name);

        Eno = (EditText) findViewById(R.id.Echange_other_name_no);
        Enew_name = (EditText) findViewById(R.id.Echange_other_name_name);
        submit = (Button) findViewById(R.id.Bchange_other_name_submit);
    ...
    class ButtonClickListener_submit implements View.OnClickListener {
        ...
        public void onClick(View v) {
            new Thread(new Runnable() {
                ...
                String new_name =Enew_name.getText().toString();
                ...
                public void run() {
                    ...
                    //检查输入的新密码格式
                    if(checkOut.check_name(new_name) == false) {
                        //提示密码格式不正确
                        Looper.prepare();
                    ...
                    try {
                        ...
                        if (response.isSuccessful()) {  //如果返回200 OK
                            ...
                            if(res.equals("Successful")){
                                ...
                                Looper.prepare();
                                Toast t = Toast.makeText(getApplicationContext(), "修改姓名成功", Toast.LENGTH_SHORT);
                                t.show();
                                Intent Iret = new Intent();  //创建意图
                                Iret.setClass(AdminChangeOtherName.this, AdminUserManagement.class);
                                AdminChangeOtherName.this.startActivity(Iret);//启动意图
                                AdminChangeOtherName.this.finish(); //关闭当前Activity
                                Looper.loop();
                            } else {
                                System.out.println("==== Failed ====");
                                Looper.prepare();
                                Toast t = Toast.makeText(getApplicationContext(), "修改姓名失败，请检查工号是否存在", Toast.LENGTH_SHORT);
                                t.show();
                                Looper.loop();
                            }
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        if (keyCode == KeyEvent.KEYCODE_BACK && event.getRepeatCount() == 0) {
            ...
            Iret.setClass(AdminChangeOtherName.this, AdminUserManagement.class);
            AdminChangeOtherName.this.startActivity(Iret);
            AdminChangeOtherName.this.finish();
            return true;
        }
